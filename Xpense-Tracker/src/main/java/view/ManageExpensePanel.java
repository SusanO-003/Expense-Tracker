...
public class ManageExpensePanel extends JPanel {
    private JTable table;
    private DefaultTableModel model;
    private String username;

    public ManageExpensePanel(String username) {
        this.username = username;
        setLayout(new BorderLayout());
        model = new DefaultTableModel(new String[]{"ID", "Date", "Description", "Category", "Amount", "Remarks"}, 0);
    ...
    private void editSelectedRow() {
        ...
        if (selectedRow >= 0) {
            ...
            if (result == JOptionPane.OK_OPTION) {
                try (Connection conn = DBConnection.getConnection()) {
                    String query = "UPDATE expenses SET date=?, description=?, category=?, amount=?, remarks=? WHERE id=?";
                    PreparedStatement ps = conn.prepareStatement(query);
